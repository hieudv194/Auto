...
# ------------------------- CẤU HÌNH -------------------------
REGION="us-east-2"  # Đã sửa từ us-east-1 thành us-east-2 theo thông báo lỗi
...
# Cấu hình instance
INSTANCE_TYPES="c6a.xlarge"  # AMD EPYC, 8 vCPU, 16GB RAM - tốt cho mining
MIN_VCPUS=4
MAX_VCPUS=4
DESIRED_VCPUS=4
...
# ------------------------- TẠO IAM ROLES -------------------------
echo "1. Tạo IAM roles và instance profiles..."

# AWS Batch Service Role (giữ nguyên)
...
# ECS Instance Role (SỬA QUAN TRỌNG - thêm ecs-tasks)
...
# Tạo Instance Profile (SỬA QUAN TRỌNG - đổi tên và cấu trúc)
INSTANCE_PROFILE_NAME="AWSBatchInstanceProfile-$ENVIRONMENT_NAME"
aws iam create-instance-profile --instance-profile-name $INSTANCE_PROFILE_NAME > /dev/null 2>&1 || echo "Instance Profile đã tồn tại, bỏ qua..."
...
echo "3. Tạo Compute Environment với instance profile đúng..."
